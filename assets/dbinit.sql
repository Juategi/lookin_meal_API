...
create table users(user_id varchar(50) unique primary key, name VARCHAR (80) NOT NULL, email unique VARCHAR (50) NOT NULL, service VARCHAR (10) NOT NULL, image VARCHAR (250) NOT NULL, username unique VARCHAR (50) NOT NULL, country VARCHAR (80) NOT NULL, recently int[], about text, token text,  checked bool); 
...
create table premium(restaurant_id integer not null, subscriptionId VARCHAR (50), PRIMARY KEY (restaurant_id), CONSTRAINT restaurant_premium__fkey FOREIGN KEY (restaurant_id) REFERENCES restaurant (restaurant_id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE);
